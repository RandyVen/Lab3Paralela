...
void Read_vector(double a[], char vec_name[], int n);
void Print_vector(double b[], char title[], int n);
void Vector_sum(double x[], double y[], double z[], int n);
float Vector_dot(double x[], double y[], double z[], double dotProduct, int n);
...
int main(void) {
   ...
   Read_n(&n);
   printf("The size of vectors is %d\n", n);
   ...
   Vector_sum(x, y, z, n);
   //dotProduct = Vector_dot(x, y, z, dotProduct, n);
   //printf("El resultado del producto punto es %f\n", dotProduct);
   Print_vector(z, "The sum is", n);
...
void Read_vector(
   ...
   int i;
   for (i = 0; i < n; i++){
      a[i] = rand()%100; //Aqui se agrego para que los vectores se crearan automaticamente
   }
...
void Print_vector(
   ...
   printf("%s\n", title);
   for (i = 0; i < n; i++)
      if(i > n-11){ //Aqui se agrego para que los vectores unicamente impriman los ultimos 10 valores
         printf("%f ", b[i]);
      }
   printf("\n");
}  /* Print_vector */
